...
Public Leaderboard, SSE Streaming)
"""
import os
import uuid
import shutil
...
# ===== SSE Event Model (в памяти, per-session) =====

def _init_session(session_id: str, repo_url: str) -> None:
    SESSIONS[session_id] = {
        "repo_url": repo_url,
        "status": "pending",
        "report": None,
        "files": {},
        "fixed_files": {},
        "events": [],          # список событий SSE
        "last_event_id": 0,    # автоинкремент id событий
        "created_at": time.time(),
        "progress": 0,         # 0–100
    }


def _next_event_id(session: dict) -> int:
    session["last_event_id"] = session.get("last_event_id", 0) + 1
...
def push_event(
    ...
    if not session:
        return
    event_id = _next_event_id(session)
    if progress is not None:
        session["progress"] = max(0, min(100, progress))
    payload = {
        "id": event_id,
        "type": event_type,  # stage/info/warning/error/complete/heartbeat
        "stage": stage,
    ...
    # ограничиваем длину истории, чтобы не раздувать память
    if len(session["events"]) > 500:
        session["events"] = session["events"][-500:]


async def sse_event_stream(session_id: str, request: Request):
    """
    SSE-стрим для одной сессии.
    Поддерживает:
    - reconnect по Last-Event-ID
    - heartbeat
    - типы событий
    """
    session = SESSIONS.get(session_id)
    if not session:
        # сразу отдадим ошибку как одно событие и закроем
        yield f"event: error\ndata: {json.dumps({'error': 'session_not_found'})}\n\n"
        return

    # начальная позиция
    last_id = 0
    # если клиент прислал Last-Event-ID — можно было бы его прочитать из заголовков,
    # но FastAPI Request не парсит это автоматически; оставим как расширение на будущее.

    heartbeat_interval = 5.0
    last_heartbeat = time.time()

    # при подключении сразу отправим текущее состояние
    yield f"event: init\ndata: {json.dumps({'status': session.get('status'), 'progress': session.get('progress', 0)})}\n\n"
    ...
    while True:
        ...
        session = SESSIONS.get(session_id)
        if not session:
            yield f"event: error\ndata: {json.dumps({'error': 'session_lost'})}\n\n"
            break
        ...
        if now - last_heartbeat >= heartbeat_interval:
            ...
            yield f"data: {json.dumps(hb_payload, ensure_ascii=False)}\n\n"
            last_heartbeat = now

        # если статус done или error и новых событий нет — можно немного подождать и завершить
        if session.get("status") in ("done", "error") and not new_events:
            await asyncio.sleep(1.0)
            # финальное событие complete/error уже должно быть отправлено из run_analysis
            break
...
def run_analysis(session_id: str, repo_url: str):
    ...
    push_event(session_id, "stage", {"message": "Старт анализа репозитория"}, stage="start", progress=1)
    try:
        push_event(session_id, "stage", {"message": "Клонирование и сканирование репозитория"}, stage="scan", progress=5)
        ...
        # ----- Git Analyzer -----
        push_event(session_id, "stage", {"message": "Git-анализ"}, stage="git_analyzer", progress=10)
        git_stats = None
        try:
            ...
            push_event(session_id, "warning", {"message": f"Ошибка Git-анализатора: {e}"}, stage="git_analyzer")

        # ----- Dependency Analyzer -----
        push_event(session_id, "stage", {"message": "Анализ зависимостей"}, stage="dependency_analyzer", progress=20)
        dep_stats = None
        try:
            ...
            push_event(session_id, "warning", {"message": f"Ошибка анализа зависимостей: {e}"}, stage="dependency_analyzer")

        # ----- Multi-Language Analyzer -----
        push_event(session_id, "stage", {"message": "Мультиязычный анализ"}, stage="multi_lang", progress=30)
        multi_lang_issues = {}
        try:
            ...
            push_event(session_id, "warning", {"message": f"Ошибка мультиязычного анализа: {e}"}, stage="multi_lang")

        # ----- Project Analyzer -----
        push_event(session_id, "stage", {"message": "Анализ структуры проекта"}, stage="project_analyzer", progress=40)
        project_issues = project_analyzer.analyze(files)

        # ----- AST Analyzer -----
        push_event(session_id, "stage", {"message": "AST-анализ Python файлов"}, stage="ast_analyzer", progress=50)
        ast_issues = {}
        ...
        # ----- Linter Runner -----
        push_event(session_id, "stage", {"message": "Запуск линтеров"}, stage="linter", progress=60)
        ...
        # ----- Prizolov Auditor -----
        push_event(session_id, "stage", {"message": "Пятиуровневый аудит Prizolov"}, stage="audit", progress=70)
        ...
        # ----- Semantic AI Layer -----
        push_event(session_id, "stage", {"message": "Семантический анализ (Yandex/LLM слой)"}, stage="semantic", progress=78)
        semantic = None
        try:
            ai = SemanticAI()
            ...
            push_event(session_id, "warning", {"message": f"Ошибка семантического анализа: {e}"}, stage="semantic")

        report["semantic"] = semantic

        # ----- Scoring Engine -----
        push_event(session_id, "stage", {"message": "Расчёт интегрального скоринга"}, stage="scoring", progress=82)
        scoring = None
        try:
            ...
            scoring = {"error": str(e)}
            push_event(session_id, "warning", {"message": f"Ошибка Scoring Engine: {e}"}, stage="scoring")

        report["scoring"] = scoring

        # ----- Smart Triage -----
        push_event(session_id, "stage", {"message": "Smart Triage — приоритезация задач"}, stage="triage", progress=86)
        triage = None
        try:
            triager = SmartTriage()
            all_issues = []
            ...
            push_event(session_id, "warning", {"message": f"Ошибка Smart Triage: {e}"}, stage="triage")

        report["triage"] = triage

        # ----- ROI Calculator -----
        push_event(session_id, "stage", {"message": "Расчёт ROI и техдолга"}, stage="roi", progress=90)
        roi = None
        try:
            ...
            push_event(session_id, "warning", {"message": f"Ошибка расчёта ROI: {e}"}, stage="roi")

        report["roi"] = roi

        # ----- Сохраняем скрытый аудит -----
        push_event(session_id, "stage", {"message": "Сохранение аудита в AuditTrail"}, stage="audit_trail", progress=93)
        ...
        session["status"] = "done"
        session["files"] = files
        push_event(session_id, "complete", {"message": "Анализ завершён"}, stage="complete", progress=100)
    except Exception as e:
        session["status"] = "error"
        session["error"] = str(e)
        push_event(session_id, "error", {"message": f"Критическая ошибка анализа: {e}"}, stage="error", progress=100)
...
async def start_scan(request: RepoRequest, background_tasks: BackgroundTasks):
    ...
    _init_session(session_id, str(request.repo_url))
    push_event(session_id, "info", {"message": "Сессия создана"}, stage="init", progress=0)
...
async def apply_fixes(session_id: str):
    ...
    push_event(session_id, "stage", {"message": "Применение автофиксов"}, stage="fix", progress=92)
    ...
    push_event(session_id, "info", {"message": "Автофиксы применены, zip готов"}, stage="fix", progress=95)
...
async def autonomous_fix(session_id: str, req: AutonomousFixRequest):
    ...
    if not files:
        raise HTTPException(500, "Нет файлов для обработки")
    push_event(session_id, "stage", {"message": "Автономный пайплайн: автофиксы"}, stage="autonomous_fix", progress=92)
    new_files = engine.format_all(files)
    ...
    try:
        ...
        if not pr_url:
            raise HTTPException(500, "Не удалось создать PR")
        push_event(session_id, "info", {"message": "Автономный PR создан", "url": pr_url}, stage="autonomous_fix", progress=100)
        return {"status": "success", "pull_request_url": pr_url}
    except Exception as e:
        push_event(session_id, "error", {"message": f"Ошибка при создании PR: {e}"}, stage="autonomous_fix", progress=100)
        raise HTTPException(500, f"Ошибка при создании PR: {str(e)}")
...
async def stream_session(session_id: str, request: Request):
    """
    SSE-стрим для фронтенда:
    - события стадий анализа
    - прогресс 0–100
    - heartbeat
    - ошибки и завершение
    """
    return StreamingResponse(
        sse_event_stream(session_id, request),
